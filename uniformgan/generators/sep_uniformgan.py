...
def train(js, dataset, epochs=200):
    ...
    noise_dim = int(js['noise_dim']) if "noise_dim" in js else 90
    device = js['device']
    batch_size = js['batch_size']
    additionals = js['additionals']
    clamp_value = float(additionals("clamp_value") or 0.01)
    privacy_epsilon = float(additionals("privacy_epsilon") or 2)
    privacy_delta = float(additionals("privacy_delta") or 1e-5)
    privacy_mM = float(additionals("privacy_mM") or 0.001)
    ...
    for i in range(epochs):
        ...
        for id_, data in enumerate(loader):
            ...
            if ((id_+1) % 50 == 0):
                with open('discriminator-log.txt', 'a') as f:
                    f.write(f"Discriminator loss at epoch {i} step {id_}: {str(loss_d)}")
            total_loss = loss_d
            total_loss.backward()
            optimizerD.step()
            ...
            if((id_ + 1) % 100 == 0):
                print("discriminator: epoch", i + 1, "step", id_ + 1, total_loss.item())
        ...
        for id_, data in enumerate(loader):
            ...
            optimizerG.step()
            if((id_ + 1) % 100 == 0):
                print("generator epoch", i + 1, "step", id_ + 1, total_loss.item(), loss_g.item(), half_check.item())
                
        if (id_ > 0):
            print("both epoch", i + 1, "step", id_ + 1, loss_g.item(), half_check.item()) # type:ignore
    return generator
