...
def gen_u(folder, generator, args=Namespace(), additionals={}):
    ...
    for file in files:
        ...
        for count in range(args.count):
            ...
            try:
                torch.save(model.state_dict(), f"models/{folder}/{file}_{generator}{args.postfix}-{ggen_count}.pt")
            ...
            for num_datas in range(4):
                samples = generators.__dict__[generator].generate(js, model, sample_total=args.gen_sample_count)
                samples.to_csv(f"datasets_synthetic/{folder}/{file}_{generator}{args.postfix}-{ggen_count}-{num_datas}.csv", index=False)
                with open('run-log.txt', 'a') as f:
                    f.write(f"\tFinished generating fake dataset {num_datas} in GEN_U\n")
            
def gen_c(folder, generator, args=Namespace(), additionals={}):
    from torch import cuda
    import torch
    js = _load(folder, generator, args, strict=True)
    js["metadata"] = json.load(open(f"datasets/{folder}/metadata.json", "r"))
    ...
    for file in files:
        ...
        for count in range(args.count):
            ...
            model = generators.__dict__[generator].train(js, dataset, epochs=args.gen_epochs)
            with open('run-log.txt', 'a') as f:
                f.write(f"Finished training for count {count} in GEN_C\n")
            try:
            ...
            for num_datas in range(4):
                ...
                samples.to_csv(f"datasets_synthetic/{folder}/{file}_{generator}{args.postfix}-{ggen_count}-{num_datas}.csv", index=False)
                with open('run-log.txt', 'a') as f:
                    f.write(f"\tFinished generating fake dataset {num_datas} in GEN_C\n")
                
def gen_nocount(folder, generator, args=Namespace(), additionals={}):
    from torch import cuda
    import torch
    js = _load(folder, generator, args, strict=True)
    js["metadata"] = json.load(open(f"datasets/{folder}/metadata.json", "r"))
    ...
    for file in files:
        ...
        print("Reading file", file)
        with open('run-log.txt', 'a') as f:
            f.write(f"Reading file {file} in GEN_NOCOUNT\n")
        dataset = pd.read_csv(f"datasets/{folder}/{file}.csv", index_col=False) # index column should be removed
        dataset = dataset.dropna()
        js['columns'] = list(dataset)

        model = generators.__dict__[generator].train(js, dataset, epochs=600) #args.gen_epochs) # 200
        try:
            torch.save(model.state_dict(), f"models/{folder}/{file}_{generator}{args.postfix}.pt")
            if "extra_pickle" not in js:
                js["extra_pickle"] = {}
            with open(f"models/{folder}/{file}_{generator}{args.postfix}.pickle", 'wb') as outfile:
                pickle.dump(js['extra_pickle'], outfile)
        except:
            print("Could not save model")
            with open('run-log.txt', 'a') as f:
                f.write("COULD NOT SAVE MODEL IN GEN_NOCOUNT\n")
                f.write(traceback.format_exc())
        for num_datas in range(4):
            with open('run-log.txt', 'a') as f:
                f.write(f"Generating fake dataset {num_datas} in GEN_NOCOUNT\n")
            samples = generators.__dict__[generator].generate(js, model, sample_total=args.gen_sample_count)
            # samples = generators.__dict__[generator].generate(js, model, sample_total=10000)
            samples.to_csv(f"datasets_synthetic/{folder}/{file}_{generator}{args.postfix}-{num_datas}.csv", index=False)
            with open('run-log.txt', 'a') as f:
                f.write(f"Finished generating fake dataset {num_datas} in GEN_NOCOUNT\n")
...
def stats_full(folder, generator, args=Namespace(), additionals={}):
    from analysis.stats import kskl_divergence, plot_correlations, table_evaluator
    js = _load(folder, generator, args)
    files = list(map(lambda x:x[0], js['files']))
    for file in files:
        ...
        print("Reading file", file)
        with open('run-log.txt', 'a') as f:
            f.write(f"Reading file {file} in STATS_full\n")
        dataset = pd.read_csv(f"datasets/{folder}/{file}.csv", index_col=False) # index column should be removed
        dataset = dataset.dropna()
        plot_corr = f"datasets_analysis/{folder}/{file}_corr.png"
        ...
        for count in range(args.count):
            ...
            for num_datas in range(4):
                with open('run-log.txt', 'a') as f:
                    f.write(f"Analyzing stats for dataset {num_datas} in STATS_full\n")
                ...
                for col in sample_dataset.columns:
                    sample_dataset[col] = sample_dataset[col].astype(dataset[col].dtype)
                plot_corr_specific = f"datasets_analysis/{folder}/{file}_{generator}{args.postfix}_corr-{ggen_count}-{num_datas}.png"
                log_path = f"datasets_analysis/{folder}/{file}_{generator}{args.postfix}_stats-{ggen_count}-{num_datas}.txt"
                log_path_error = f"datasets_analysis/{folder}/{file}_{generator}{args.postfix}_error-{ggen_count}-{num_datas}.stats.log"
                logger = open(log_path, "w")
                error_logger = open(log_path_error, "w")
                kskl_divergence(js, dataset, sample_dataset, logger, error_logger)
                plot_correlations(js, dataset, sample_dataset, logger, plot_corr, plot_corr_specific)
...
def privacy(folder, generator, args=Namespace(), additionals={}):
    from analysis.privacy import dcr_modified as dcr
    js = _load(folder, generator, args)
    files = list(map(lambda x:x[0], js['files']))
...
def privacy_full(folder, generator, args=Namespace(), additionals={}):
    from analysis.privacy import dcr_modified as dcr
    js = _load(folder, generator, args)
    files = list(map(lambda x:x[0], js['files']))
    ...
    for file in files:
        ...
        print("Reading file", file)
        with open('run-log.txt', 'a') as f:
            f.write(f"Reading file {file} in PRIVACY_NOCOUNT\n")
        dataset = pd.read_csv(f"datasets/{folder}/{file}.csv", index_col=False) # index column should be removed
        dataset = dataset.dropna()
        
        for count in range(args.count):
            print(f"Privacy for {folder} with {generator} #", count)
            ggen_count = str(count).zfill(2)
            
            for num_datas in range(4):
                with open('run-log.txt', 'a') as f:
                    f.write(f"Analyzing fake dataset number {num_datas} in privacy_full\n")
                sample_dataset = pd.read_csv(f"datasets_synthetic/{folder}/{file}_{generator}{args.postfix}-{ggen_count}-{num_datas}.csv", index_col=False)
                sample_dataset = sample_dataset.dropna()
                for col in sample_dataset.columns:
                    sample_dataset[col] = sample_dataset[col].astype(dataset[col].dtype)
                log_path = f"datasets_analysis/{folder}/{file}_{generator}{args.postfix}_privacy-{ggen_count}-{num_datas}.txt"
                log_path_error = f"datasets_analysis/{folder}/{file}_{generator}{args.postfix}_error-{ggen_count}-{num_datas}.privacy.log"
                logger = open(log_path, "w")
                error_logger = open(log_path_error, "w")
                dcr(js, dataset, sample_dataset, logger, error_logger)
                logger.close()
...
def eval(folder, generator, args=Namespace(), additionals={}):
    ...
    for file, test_file in files:
        ...
        for count in range(args.count):
            ...
            logger = open(log_path, "w")
            error_logger = open(log_path_error, "w")
            simple_eval(js, dataset, test_dataset, sample_dataset, logger, error_logger, verbose=args.verbose)
            logger.close()
            error_logger.close()
            
def eval_full(folder, generator, args=Namespace(), additionals={}):
    from analysis.eval import simple_eval
    js = _load(folder, generator, args)
    files = js['files']
    ...
    for file, test_file in files:
        ...
        for count in range(args.count):
            ...
            for num_datas in range(4):
                with open('run-log.txt', 'a') as f:
                    f.write(f"Analyzing fake dataset number {num_datas} in eval_full\n")
                sample_dataset = pd.read_csv(f"datasets_synthetic/{folder}/{file}_{generator}{args.postfix}-{ggen_count}-{num_datas}.csv", index_col=False)
                sample_dataset = sample_dataset.dropna()
                for col in sample_dataset.columns:
                    sample_dataset[col] = sample_dataset[col].astype(dataset[col].dtype)
                log_path = f"datasets_analysis/{folder}/{file}_{generator}{args.postfix}_eval-{ggen_count}-{num_datas}.txt"
                log_path_error = f"datasets_analysis/{folder}/{file}_{generator}{args.postfix}_error-{ggen_count}-{num_datas}.eval.log"
                logger = open(log_path, "w")
                error_logger = open(log_path_error, "w")
                simple_eval(js, dataset, test_dataset, sample_dataset, logger, error_logger, verbose=args.verbose)
                logger.close()
