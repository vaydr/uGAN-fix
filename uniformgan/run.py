                ...
                for generator in arg_generators:
                    try:
                        ...
                        with open('run-log.txt', 'a') as f:
                            f.write(f"An error occurred with {action} for generator {generator}\n")
                            f.write(f"Error: {e}\n")
...
if __name__ == "__main__":
    ...
    print("****************")
    print("Add --verbose for more debugging")
    ...
    parser.add_argument('--production', metavar='production', type=str2bool, nargs='?',
                        const=True, default=False, help="Activate production mode.")
    ...
    if (not args.test and not args.production):
        print("You must select either --production or --test to proceed!")
        exit() 
    if (not args.test):
